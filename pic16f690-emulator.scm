 ...
 ; (define TMR0 1)
   ...
   (let ([w-reg 0]
		 ...
		 [call-stack-head 0]
		 [config-mem (make-memory 14 (make-range #x2000 #x3fff) #x80 ; configuration memory
								  '(#x2000 #x2001 #x2002 #x2003 #x2007 #x2008))] ;
		 [eeprom (make-memory 8 (make-range 0 #xff) #x100)] ; Data EEPROM. r/w.
...
;			 [mask (ref *mask* b)])
	 ...
	 (define (snapshot)
	   `(PIC16F690
		 ...
		 (config ,(clone-vec [config-mem'vec]))
		 (eeprom ,(clone-vec [eeprom'vec]))
